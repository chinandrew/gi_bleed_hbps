...
class BayesBridge():
    ...
    def gibbs(self, n_iter, n_burnin=0, thin=1, seed=None,
        ...
        for mcmc_iter in range(1, n_iter + 1):
            ...
            coef, info = self.update_regress_coef(
                coef, obs_prec, gscale, lscale, options.coef_sampler_type, dt, rr, unit_v
            )
        ...
        mcmc_info = {
            ...
            'dot_count': self.model.design.dot_count,
            'Tdot_count': self.model.design.Tdot_count,
            'grad_evals': self.model.compute_loglik_and_gradient(None, return_n_calls=True)
        }
    ...
    def update_regress_coef(self, coef, obs_prec, gscale, lscale, sampling_method, dt, rr, unit_v):
        ...
        elif sampling_method in ['hbps', 'hbpsnuts']:
            coef, info = self.reg_coef_sampler.sample_by_hbps(
                coef, gscale, lscale, self.model, method=sampling_method, dt=dt, unit_v=unit_v,
            )
        elif sampling_method in ['bps']:
            coef, info = self.reg_coef_sampler.sample_by_bps(
                coef, gscale, lscale, self.model, method=sampling_method, t=dt, rr=rr
            )
        else:
