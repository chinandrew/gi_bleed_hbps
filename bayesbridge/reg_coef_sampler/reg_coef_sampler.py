...
from .hamiltonian_bps.bps import BPSSampler
from .hamiltonian_bps.nuts import HBPSNoUTurnSampler
from .hamiltonian_bps.hbps import HBPSSampler
...
class SparseRegressionCoefficientSampler():
    ...
    def sample_by_hbps(
            self, beta, gscale, lscale, model, method='hbpsnuts', dt=1, unit_v=False):
        ...
        v = np.random.normal(size=beta_precond.shape[0])
        v = v / np.linalg.norm(v) if unit_v else v
        inertia = np.random.exponential(1)
        if method == 'hbpsnuts':
            model.design.memoize_dot(True)
            beta_precond, info = HBPSNoUTurnSampler(f).generate_next_state(dt, beta_precond, p=v, inertia=inertia, max_tree_height=16, unit_v=unit_v)
            model.design.memoize_dot(False)
        elif method == 'hbps':
            model.design.memoize_dot(True)
            beta_precond, info = HBPSSampler.generate_next_state(f, beta_precond, v, inertia, dt)
            model.design.memoize_dot(False)
        else:
            raise NotImplementedError()
        beta = beta_precond * precond_scale
    ...
    def sample_by_bps(
            self, beta, gscale, lscale, model, method='bps', t=1, rr=1, unit_v=False):
        ...
        if method == 'bps':
            ...
            v = v/np.linalg.norm(v) if unit_v else v
            model.design.memoize_dot(True)
            beta_precond, num_bounces = BPSSampler(f).generate_next_state(beta_precond, v, t, rr, False, unit_v)
            model.design.memoize_dot(False)
            info = {'num_bounces': num_bounces}
        else:
            raise NotImplementedError()
        beta = beta_precond * precond_scale
        self.regcoef_summarizer.update(beta, gscale, lscale)
